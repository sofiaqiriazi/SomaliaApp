...
shinyServer(function(input, output) {
  ...
  mydata <- reactive({
    ...
    #Model on Rainfall influence on conflicts
    #Change in Conflict 
    DFall <-function(t) RF/(Cf[t]+1)
    ...
    #Replica rainfall long
    longrain <- data.frame(
      Period=rep((0:np),6), 
      Population = c(Cf), 
      Indicator=rep(c("Conflict"), 
                    each=np+1))
    widerain <- cbind(Cf,r0)
    ...
    list(long=long, wide=wide, short = longrain, tall=widerain)
 ...
 #output graph rainfall conflict
